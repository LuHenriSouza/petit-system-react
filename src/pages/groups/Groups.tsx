import { GroupService, IGroup, IProduct, ProductService } from '../../shared/services/api';
import { Alert, Box, Button, Dialog, DialogActions, DialogContent, DialogTitle, Fab, Grid, Icon, Pagination, Paper, Table, TableBody, TableCell, TableHead, TableRow, TextField, useTheme } from "@mui/material";
...
export const Groups: React.FC = () => {
	...
	const listGroups = async () => {
		const response = await GroupService.getAll(Number(groupPage), groupSearch, GROUP_ROW_LIMIT);
	...
	const listProdsInGroup = async () => {
		const response = await GroupService.getProdsByGroup(groupSelectedRow, Number(prodInGroupPage), prodInGroupSearch, PRODGROUP_ROW_LIMIT);
		if (response instanceof Error) return alert(response.message);
	...
	const listProducts = async () => {
		await ProductService.getAll(Number(prodPage), prodSearch, PROD_ROW_LIMIT)
			.then((result) => {
	...
	const handleGroupRowClick = async (id: number) => {
		...
		setSearchParams((old) => { old.delete('prodInGroupSearch'); return old; });
	}
	...
	const handlePutProdInGroup = async () => {
		const response = await GroupService.putProdInGroup(groupSelectedRow, prodSelected);
		if (response instanceof Error) {
			setErrorExists(true);
		} else {
			Swal.fire({
	...
	return (
		<LayoutMain title="Grupos" subTitle="Crie e gerencie grupos de produtos">
			<Grid container>
					...
					<Paper sx={{ backgroundColor: '#fff', px: 3, py: 1, mr: 5, mb: 1 }} variant="elevation">
						<Box display={'flex'} justifyContent={'space-between'}>
							<TextField
								...
								autoComplete="off"
							/>
							<Button variant="contained" onClick={handleNewGroup}><AddIcon /></Button>
						</Box>
					</Paper>
					<Paper sx={{ backgroundColor: '#fff', px: 3, py: 1, mr: 5, minHeight: 600 }} variant="elevation">
						<TextField
							size="small"
							placeholder={'Pesquisar Grupo'}
							value={groupSearch}
							onChange={(event) => setSearchParams((old) => { old.set('groupSearch', event.target.value); return old; })}
							autoComplete="off"
							fullWidth
						/>
						<Box minHeight={470}>
									...
									{groupRows.map(group => (
										...
										>
											...
											<TableCell align='right'>
												<Fab size="medium" color="error" aria-label="add" onClick={() => handleDeleteGroup(group.id, group.name)}>
													<Icon>delete</Icon>
												</Fab>
											</TableCell>
										</TableRow >
							...
							<TableRow>
								<TableCell colSpan={3}>
									<Pagination
										...
										onChange={(_, newPage) => setSearchParams(old => { old.set('groupPage', newPage.toString()); return old; })}
									/>
								</TableCell>
							</TableRow>
						)}
				...
				<Grid item xs={8}>
					<Paper sx={{ backgroundColor: '#fff', mr: 4, px: 3, py: 1, mb: 1 }} variant="elevation">
						<Box display={'flex'} justifyContent={'space-between'}>
							<Button variant='outlined' onClick={handleCleanClick} disabled={!groupSelectedRow}>
								<UndoIcon fontSize='small' sx={{ mr: 1 }} />
								Limpar Seleção
							</Button>
							<Button variant='outlined' onClick={handleClickOpen} disabled={!groupSelectedRow}>
								<AddIcon sx={{ mr: 1 }} />
								Adicionar Produto
							</Button>
						</Box>
					</Paper>
					<Paper sx={{ backgroundColor: '#fff', mr: 4, px: 3, py: 1, minHeight: 600 }} variant="elevation">
						{!NAProd && (
							<TextField
								size="small"
								placeholder={'Pesquisar Produto'}
								value={prodInGroupSearch}
								onChange={(event) => setSearchParams((old) => { old.set('prodInGroupSearch', event.target.value); return old; })}
								autoComplete="off"
								fullWidth
							/>
						)}
						{NAProd && (<Alert icon={false} color='warning' sx={{ mt: 2 }}>Selecione um grupo</Alert>)}
						<Box minHeight={470}>
							{!NAProd && (
								<Table>
									<TableHead>
						...
						{(prodGroupTotalCount > 0 && prodGroupTotalCount > PRODGROUP_ROW_LIMIT) && (
							<TableRow>
								<TableCell colSpan={3}>
									<Pagination
										page={Number(prodInGroupPage)}
										count={Math.ceil(prodGroupTotalCount / PRODGROUP_ROW_LIMIT)}
										onChange={(_, newPage) => setSearchParams(old => { old.set('prodInGroupPage', newPage.toString()); return old; })}
									/>
								</TableCell>
							</TableRow>
						)}
			...
			<Dialog
				...
				sx={{
					"& .MuiDialog-paper":
						{ backgroundColor: "#fff", maxWidth: 450 }
				}}>
				...
				<DialogContent>
					...
					{errorExists && (<Alert severity='warning'>Produto ja pertence ao grupo</Alert>)}
							...
							<TableRow>
								<TableCell colSpan={3}>
									<Pagination
										...
										onChange={(_, newPage) => setSearchParams(old => { old.set('prodPage', newPage.toString()); return old; })}
									/>
								</TableCell>
							</TableRow>
						</Box>
					)}
					<Button fullWidth variant='contained' disabled={!prodSelected} onClick={handlePutProdInGroup}>Adicionar</Button>
				</DialogContent>
				<DialogActions>
